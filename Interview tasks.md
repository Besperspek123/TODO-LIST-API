...
История покупок:
GET /api/users/purchases - для получения списка всех покупок пользователя;
POST /api/purchases - для добавления новой покупки (для пользователя);
GET /api/purchases/{id} - для получения информации о конкретной покупке по ее ID;
DELETE /api/purchases/{id} - для удаления покупки (для пользователя).
Отзывы и оценки:
...
POST /api/ratings - для добавления новой оценки (для пользователя).
...
PUT /api/users/{id}/balance - для пополнения баланса